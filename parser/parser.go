...
import (
	...
	lx "github.com/theawakener0/MkLang/lexer"
...
func New(l *lx.Lexer) *Parser {
	...
	if features.PrefixPostfixIncDec {
	...
	if features.ForLoops {
		p.registerPrefix(tk.FOR, p.parseForExpression)
	}
	if features.InfiniteLoops {
...
func (p *Parser) parseStatement() ast.Statement {
	...
	case tk.IDENT:
		...
		} else if tk.Current.Features.CompoundAssignment &&
			(p.peekTokenIs(tk.INCASSIGN) || p.peekTokenIs(tk.DECDASSIGN) ||
...
func isAssignOp(tok tk.TokenType) bool {
	...
	case tk.INCASSIGN, tk.DECDASSIGN, tk.MLTASSIGN, tk.DIVASSIGN:
		return tk.Current.Features.CompoundAssignment
	default:
		return false
	}
}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	p.skipPeekSemicolons()
	if tk.Current.Features.ElseIf && p.peekTokenIs(tk.ELSEIF) {
		p.nextToken()
