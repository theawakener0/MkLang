...
// builtinRegistry holds every builtin under its canonical name. The active
// `builtins` map (used for lookup) is built from this registry and the
// configured spellings in tk.Current.Builtins, so a language can rename,
// remove, or re-map builtins.
var builtinRegistry = map[string]*obj.Builtin{
	"len": {
...
// builtins is the active builtin map: configured spelling -> builtin. It is
// rebuilt whenever the language spec changes.
...
// activateBuiltins rebuilds the active builtin map from tk.Current.Builtins.
func activateBuiltins() {
	builtins = make(map[string]*obj.Builtin)
	for spelling, canonical := range tk.Current.Builtins {
		if canonical == "matrix" && !tk.Current.Features.Matrices {
			continue
		}
		if b, ok := builtinRegistry[canonical]; ok {
			builtins[spelling] = b
		}
	}
}
